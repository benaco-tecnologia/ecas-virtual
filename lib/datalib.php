...
function get_users($get=true, $search='', $confirmed=false, $exceptions='', $sort='firstname ASC',
    ...
    if ($get) {
        return get_records_select('user', $select . " AND lastlogin > 0", $sort, $fields, $page, $recordsperpage);
    } else {
        return count_records_select('user', $select . " AND lastlogin > 0");
    }
}
...
/// warning: will return UNCONFIRMED USERS
    return get_records_sql("SELECT id, username, email, firstname, lastname, city, country, lastaccess, confirmed, mnethostid, tipo_usuario
                              FROM {$CFG->prefix}user
                             WHERE $select AND lastlogin > 0 $sort", $page, $recordsperpage);
