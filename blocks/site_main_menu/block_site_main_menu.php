...
/// extra fast view mode
        if (!$isediting) {
			...
			$html  = '<script type="text/javascript">
					  //<![CDATA[
						function certificados(url)
						{
							mywindow = window.open(url, \'\',\'status=0,width=750,height=500,scrollbars=1,resizable=0,location=0\');
						}	
					 //]]>
					 </script>
					 ';
			require_login(); 	
			if (is_numeric(get_field('user', 'username', 'id', $USER->id)))
			{	 				
				$html .= '<a href="http://alumnos.ecas.cl/certificados/v2/solicitud.asp?usrcod='.get_field('user', 'username', 'id', $USER->id).'&h='.($USER->sesskey).md5(time()).'" onclick="certificados(this.href); return false;">Solicitar Certificado</a>';
			}
			...
			if (is_numeric(get_field('user', 'username', 'id', $USER->id)))
			{	 				
				$html .= '<a href="http://ecasvirtual.cl/mispagos/boletas.php?h='.($USER->sesskey).md5(time()).md5(($USER->sesskey)).'&u='.get_field('user', 'username', 'id', $USER->id).'" onclick="mis_pagos(this.href); return false;">Mis Pagos</a>';
			}
			$this->content->items[] = $html;
			$this->content->icons[] = '';
