...
function assignment_cron () {
	...
	$starttime = $endtime - 24 * 3600 * 20;   /// 7 days earlier
    ...
    if ($submissions = assignment_get_unmailed_submissions($starttime, $endtime)) {
        ...
        $timenow = time();
		echo "assigment cron: se encontraron " . count($submissions) . " notificaciones pendientes.";
...
//			echo "!email_to_user($user, $teacher, $postsubject, $posttext, $posthtml))";
			//@mail("daniel.fuentes.b@gmail.com", $postsubject, $posthtml);
		...
		echo "assigment cron: fueron enviados $i emails con notificaciones. ($j errores)";

        $USER = $realuser;
        course_setup(SITEID); // reset cron user language, theme and timezone settings

    } else {
		echo "assigment cron: no se encontraron notificaciones pendientes.";
	}
